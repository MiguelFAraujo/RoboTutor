from django.urls import path, include
from django.contrib.auth import views as auth_views
from . import views, billing

urlpatterns = [
    path('', views.landing, name='landing'),
    path('auth/google/', views.google_login_entry, name='google_login_entry'),
    path('academy/', views.academy_catalog, name='academy'),
    path('academy/<slug:slug>/', views.academy_pack_detail, name='academy_pack'),
    path('academy/<slug:slug>/buy/', views.academy_pack_purchase, name='academy_pack_purchase'),
    path('academy/pdfs/<slug:slug>/', views.academy_pdf_download, name='academy_pdf_download'),
    path('academy/orders/<int:order_id>/success/', views.academy_order_success, name='academy_order_success'),
    path('academy/orders/<int:order_id>/download/', views.academy_bundle_download, name='academy_bundle_download'),
    path('chat/', views.index, name='index'),
    path('chat/', views.index, name='chat'),  # Alias for 'chat'
    path('login/', auth_views.LoginView.as_view(), name='login'),
    path('logout/', auth_views.LogoutView.as_view(), name='logout'),
    path('api/chat', views.chat_api, name='chat_api'),
    path('history/<int:conversation_id>/', views.get_conversation_history, name='history'),
    path('conversation/<int:conversation_id>/delete/', views.delete_conversation, name='delete_conversation'),
    path('register/', views.register, name='register'),
    path('subscribe/', billing.create_checkout_session, name='subscribe'),
    path('webhook/', billing.stripe_webhook, name='webhook'),
    
    # Project Collection
    path('projects/', views.projects_list, name='projects'),
    path('api/projects/create', views.create_project, name='create_project'),
    path('api/projects/<int:project_id>/status', views.update_project_status, name='update_project_status'),
    path('api/projects/<int:project_id>/delete', views.delete_project, name='delete_project'),
]


